...
start_run :: proc(mem: ^memory.Memory, scr: ^screen.Screen($W, $H)) {
    ...
    for {
        assert(
            itp.program_counter % 2 == 0,
            fmt.aprintf(
                "Program counter stopped on an odd address: %X\n",
                itp.program_counter,
            ),
        )
...
create_instruction_cache :: proc(
    ...
    for i := 0; i < mem.program_length; i += STEP_SIZE {
        ...
        argument_byte := memory.get_at(
            mem,
            u16(i) + itp.program_counter + NEXT_ADDR,
        )
